...
public abstract class LightChannelTypeProvider {
    private final Set<LightChannelDef> supported;

    public LightChannelTypeProvider(LightChannelDef... supported) {
    ...
    public final ChannelType provideChannelType(LightChannelDef channelType, ListEntitiesLightResponse rsp) {
        if (!supported.contains(channelType)) {
            throw new IllegalArgumentException("Actual channel type " + channelType + " is not supported by " + getClass().getName());
        }
        return provideType(provideChannelTypeId(channelType, rsp), channelType, rsp);

    }

    protected abstract ChannelType provideType(ChannelTypeUID channelTypeUid, LightChannelDef channelType, ListEntitiesLightResponse rsp);
    ...
    public static final LightChannelTypeProvider NOT_IMPLEMENTED = new LightChannelTypeProvider(LightChannelDef.values()) {
        @Override
        protected ChannelType provideType(ChannelTypeUID channelTypeUid, LightChannelDef channelType, ListEntitiesLightResponse rsp) {
    ...
    public static final LightChannelTypeProvider ON_OFF = new LightChannelTypeProvider(LightChannelDef.ON_OFF) {
        @Override
        protected ChannelType provideType(ChannelTypeUID channelTypeUid, LightChannelDef channelType, ListEntitiesLightResponse rsp) {
    ...
    public static final LightChannelTypeProvider COLOR_MODE = new LightChannelTypeProvider(LightChannelDef.COLOR_MODE) {
        @Override
        protected ChannelType provideType(ChannelTypeUID channelTypeUid, LightChannelDef channelType, ListEntitiesLightResponse rsp) {
            final var modeOptions = rsp.getSupportedColorModesList().stream().map(ColorMode::decodeFromBitMask)
                    .sorted().map(mode -> new StateOption(mode.name(), mode.description())).toList();
        ...
        @Override
        protected ChannelTypeUID provideTypeId(LightChannelDef channelType, ListEntitiesLightResponse rsp) {
            final var modesSetSubtype = rsp.getSupportedColorModesList().stream()
    ...
    public static final LightChannelTypeProvider BRIGHTNESS = new LightChannelTypeProvider(LightChannelDef.MASTER_BRIGHTNESS, LightChannelDef.COLOR_BRIGHTNESS) {
        @Override
        protected ChannelType provideType(ChannelTypeUID channelTypeUid, LightChannelDef channelType, ListEntitiesLightResponse rsp) {
    ...
    public static final LightChannelTypeProvider RGBW = new LightChannelTypeProvider(LightChannelDef.RED, LightChannelDef.GREEN, LightChannelDef.BLUE, LightChannelDef.WHITE) {
        @Override
        protected ChannelType provideType(ChannelTypeUID channelTypeUid, LightChannelDef channelType, ListEntitiesLightResponse rsp) {
            return extendSystemStateChannelType(DefaultSystemChannelTypeProvider.SYSTEM_BRIGHTNESS, channelType)
                    .withStateDescriptionFragment(zeroToOneStateDescr().build())
    ...
    public static final LightChannelTypeProvider COLD_WARM_WHITE = new LightChannelTypeProvider(LightChannelDef.COLD_WHITE, LightChannelDef.WARM_WHITE) {
        @Override
        protected ChannelType provideType(ChannelTypeUID channelTypeUid, LightChannelDef channelType, ListEntitiesLightResponse rsp) {
            ...
            return extendSystemStateChannelType(DefaultSystemChannelTypeProvider.SYSTEM_BRIGHTNESS, channelType, kelvinFormatted + "k")
                    .withStateDescriptionFragment(zeroToOneStateDescr().build())
                    .withTag(channelType.name() + "-" + kelvinFormatted + "K")
                    .build();
        }
        ...
        protected ChannelTypeUID provideTypeId(LightChannelDef channelType, ListEntitiesLightResponse rsp) {
            ...
            return extendSystemChanelTypeUid(DefaultSystemChannelTypeProvider.SYSTEM_BRIGHTNESS, channelType, kelvinFormatted + "k");
        }

    };

    public static final LightChannelTypeProvider COLOR_TEMPERATURE = new LightChannelTypeProvider(LightChannelDef.COLOR_TEMPERATURE) {
        @Override
        protected ChannelType provideType(ChannelTypeUID channelTypeUid, LightChannelDef channelType, ListEntitiesLightResponse rsp) {
            var kelvinSorted = Stream.of(rsp.getMaxMireds(), rsp.getMinMireds())
                    .map(ColorTempUtil::miredToKelvin)
                    .sorted()
                    .toList();
            ...
            return extendSystemStateChannelType(DefaultSystemChannelTypeProvider.SYSTEM_COLOR_TEMPERATURE_ABS, channelType, ctSubtype)
        ...
        @Override
        protected ChannelTypeUID provideTypeId(LightChannelDef channelType, ListEntitiesLightResponse rsp) {
            var ctSubtype = Stream.of(rsp.getMaxMireds(), rsp.getMinMireds())
                    .map(ColorTempUtil::miredToKelvin)
                    .sorted()
                    .map(ColorTempUtil::formatKelvin)
                    .collect(Collectors.joining("-"));
            return extendSystemChanelTypeUid(DefaultSystemChannelTypeProvider.SYSTEM_COLOR_TEMPERATURE_ABS, channelType, ctSubtype);
        }

    };

    public static final LightChannelTypeProvider DURATION = new LightChannelTypeProvider(LightChannelDef.TRANSITION_LENGTH, LightChannelDef.FLASH_LENGTH) {

        @Override
        protected ChannelType provideType(ChannelTypeUID channelTypeUid, LightChannelDef channelType, ListEntitiesLightResponse rsp) {
            var stateDescr = StateDescriptionFragmentBuilder.create()
                    .withMinimum(BigDecimal.ZERO)
                    .withMaximum(BigDecimal.valueOf((long) Integer.MAX_VALUE * 2))
    ...
    private static ChannelTypeUID extendSystemChanelTypeUid(ChannelType base, LightChannelDef type, @Nullable String... subtypes) {
